...
# Get the JSON object
JSON_OBJ_STR=$(echo $JSON_EVENT_STR | jq '.[0].object')

# Lets extract out several key values
POD_NAME=$(echo "$JSON_OBJ_STR" | jq '.metadata.name')
...
TERMINATED_EXITCODE=$(echo "$JSON_OBJ_STR" | jq '.status.containerStatuses[0].state.terminated.exitCode')
TERMINATED_REASON=$(echo "$JSON_OBJ_STR" | jq '.status.containerStatuses[0].state.terminated.reason')
